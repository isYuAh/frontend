...
const handleDeleteTicket = async (ticket: Ticket) => {
    Ticket.delete(props.id, ticket.id, {serverEndpoint: devConfig.serverEndpoint}).then(() => {
        setMessage({
        type:'success',
        message: '成功删除加分条信息',
        });
...
const handleSubmitEdit = async (ticket: Ticket) => {
    try {
        Ticket.update(props.id, ticket.id, {
...
<template>
  <td class="px-6 py-4">{{ temp.id }}</td>
    ...
    <td class="px-6 py-4">
    <template v-if="editing">
        <select v-model="temp.type" class="mx-1 py-2 border rounded">
    ...
    <template v-if="editing">
        <input-text
        v-model="temp.points"
        class="mx-1 w-32 py-2" name="points"
        type="number"
        :step="100"
    ...
    <td class="px-6 py-4">{{ temp.date }}</td>
    <td class="px-6 py-4">
    <template v-if="editing">
        <a class="text-primary dark:text-primary-200 m-1 underline" href="?" @click.prevent="handleCancel">取消</a>
        <a class="text-primary dark:text-primary-200 m-1 underline" href="?" @click.prevent="handleSubmitEdit(temp)">提交</a>
    </template>
    <template v-else>
        <a class="text-primary dark:text-primary-200 m-1 underline" href="?" @click.prevent="editing = true">修改</a>
        <a class="m-1 text-red-500 underline dark:text-red-200" href="?" @click.prevent="handleDeleteTicket(temp)">删除</a>
    </template>
    </td>
</template>